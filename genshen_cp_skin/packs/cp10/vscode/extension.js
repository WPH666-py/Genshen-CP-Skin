// 原神 CP 壁纸套件 10 — 雷电影 × 八重神子 · 单张素材三种呈现 (VSCode / Trae / CodeX / Cursor / Windsurf)
...
const REPO_URL = 'https://github.com/WPH666-py/Genshen-skin-CP10.git';
const EXT_ID = 'wp666.genshen-skin-cp10';
...
function detectPython() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp10').get('pythonPath', ''));
...
function findRepo() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp10').get('repoPath', ''));
  const cands = [
    cfg,
    process.env.GENSHIN_CP1_REPO,
    path.join(os.homedir(), 'Genshen-skin-CP10'),
    process.env.USERPROFILE ? path.join(process.env.USERPROFILE, 'Genshen-skin-CP10') : null,
  ...
  for (const c of cands) {
    try {
      if (fs.existsSync(path.join(c, 'src', 'genshen_skin_cp10', 'cli.py'))) return c;
...
function buildInvocation(sub) {
  ...
  if (repo) {
    env.PYTHONPATH = (env.PYTHONPATH ? env.PYTHONPATH + path.delimiter : '') +
      path.join(repo, 'src');
    return { py: py, args: ['-m', 'genshen_skin_cp10.cli'].concat(sub), cwd: repo, env: env };
  }
  // pip 安装形态: 直接用模块
  return { py: py, args: ['-m', 'genshen_skin_cp10.cli'].concat(sub), cwd: os.homedir(), env: env };
}

function envError(msg) {
  vscode.window.showWarningMessage(
    msg + ' 请先安装: pip install ' + 'genshen-skin-cp10' +
    '  (或克隆 ' + REPO_URL + ' 到 ' + path.join(os.homedir(), 'Genshen-skin-CP10') + ')'
  );
}
...
function spawnGui(moduleName, label) {
  ...
  const child = spawn(exe, ['-m', 'genshen_skin_cp10.' + moduleName], {
    cwd: inv.cwd,
    env: inv.env,
    detached: true,
    stdio: 'ignore',
...
// 本套件只有一张素材(雷电影 × 八重神子 双人合影), 因此画廊给的是同一张的
// 三种呈现方式。ids 必须与 characters/cp10_pair.py 的 MODES 一致。
const CARDS = [
  { mode: 'single1', t: '鸟居 · 卡片', img: 'thumb-single1.png', d: '模糊背景 + 居中卡片' },
  { mode: 'single2', t: '沙发 · 卡片', img: 'thumb-single2.png', d: '模糊背景 + 居中卡片' },
  { mode: 'single3', t: '雷鸣 · 卡片', img: 'thumb-single3.png', d: '模糊背景 + 居中卡片' },
  { mode: 'cover1', t: '鸟居 · 满屏', img: 'thumb-cover1.png', d: 'cover 铺满整屏' },
  { mode: 'cover2', t: '沙发 · 满屏', img: 'thumb-cover2.png', d: 'cover 铺满整屏' },
  { mode: 'cover3', t: '雷鸣 · 满屏', img: 'thumb-cover3.png', d: 'cover 铺满整屏' },
  { mode: 'showall1', t: '鸟居 · 完整', img: 'thumb-showall1.png', d: '不裁切, 两侧留边' },
  { mode: 'showall2', t: '沙发 · 完整', img: 'thumb-showall2.png', d: '不裁切, 两侧留边' },
  { mode: 'showall3', t: '雷鸣 · 完整', img: 'thumb-showall3.png', d: '不裁切, 两侧留边' }
];
...
function renderHtml(thumb, hasPython) {
  ...
  return '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>' +
    ...
    '</div>' +
    '<div class="foot">原神 CP 壁纸套件 10 · 雷电影 × 八重神子 v0.1.0<br/>' +
...
function activate(ctx) {
  ...
  reg('genshencp10.set1', () => runCli(['1'], '已切换: 鸟居'));
  reg('genshencp10.set2', () => runCli(['2'], '已切换: 沙发'));
  reg('genshencp10.set3', () => runCli(['3'], '已切换: 雷鸣'));
  reg('genshencp10.setCard', () => runCli(['card'], '已切换: 卡片式'));
  reg('genshencp10.setCover', () => runCli(['cover'], '已切换: 满屏'));
  reg('genshencp10.setShowall', () => runCli(['showall'], '已切换: 完整不裁'));
  reg('genshencp10.setRandom', () => runCli(['random'], '已随机换一张!'));
  reg('genshencp10.generateAll', () => runCli(['all'], '已生成全部样式!'));
  reg('genshencp10.openSwitcher', () => spawnGui('switcher', '壁纸切换器'));
  reg('genshencp10.startPet', () => spawnGui('pet', '桌面桌宠'));
  reg('genshencp10.showInfo', () => {
    const inv = buildInvocation(['info']);
    if (inv.error) return envError(inv.error);
    const term = vscode.window.createTerminal({ name: '原神CP10 自检', cwd: inv.cwd, env: inv.env });
    term.show();
    term.sendText('"' + inv.py + '" ' + inv.args.map((a) => '"' + a + '"').join(' '));
  });
  reg('genshencp10.openGallery', () =>
    vscode.commands.executeCommand('workbench.view.extension.genshencp10')
  );

  ctx.subscriptions.push(
    vscode.window.registerWebviewViewProvider('genshencp10.gallery', new GalleryProvider(), {
