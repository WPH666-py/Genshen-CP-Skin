...
def pkg_root():
    """包目录 .../genshen_skin_cp10(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp10.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #f3edf7;
  --bg-layer-3: #e4d9ee;
  --label-primary: #241a2c;
  --label-secondary: #514060;
  --label-tertiary: #816f90;
  --brand-primary: #8a5fb0;
  --accent: #8a5fb0;
  --primary: #8a5fb0;
  --interactive-bg-hover-solid: #f3e4ec;
  --interactive-bg-active: #e6d0e0;
  --border-l1: #eaddf0;
  --border-l2: #d6c3e2;
  --border: #d6c3e2;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP10</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP10", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP10", mascot.get("dark")),
    }
