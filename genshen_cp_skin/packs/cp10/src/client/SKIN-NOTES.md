...
本目录的 `genshen-cp10.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp10.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp10 deepking          # 导出 ~/.genshen-cp10/deepking/*.skin.json
  ```
...
```bash
genshen-cp10 deepking --check    # 应输出「通过」
genshen-cp10 deepking --what     # 看转换器实际会取到什么值
```
...
- `mascot-cp10-light.jpg` —— 取「鸟居」那张的樱下双人, 配浅色界面
- `mascot-cp10-dark.jpg` —— 取「雷鸣」那张的黑底紫电, 配深色界面
