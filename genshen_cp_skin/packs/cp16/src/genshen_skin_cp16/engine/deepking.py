...
def pkg_root():
    """包目录 .../genshen_skin_cp16(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp16.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #eef4f8;
  --bg-layer-3: #dbe8f1;
  --label-primary: #1f2b3d;
  --label-secondary: #48586e;
  --label-tertiary: #7c8b9e;
  --brand-primary: #c9a227;
  --accent: #c9a227;
  --primary: #c9a227;
  --interactive-bg-hover-solid: #f5ebd2;
  --interactive-bg-active: #ebdcb0;
  --border-l1: #ece2cd;
  --border-l2: #d8c9a8;
  --border: #d8c9a8;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP16</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP16", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP16", mascot.get("dark")),
    }
