...
try:
    from genshen_skin_cp23.engine import skin_core as sc
...
def make_thumbs():
    ...
    # 拼版总览图: 按实际样式数量排成一行(本套件 3 个), 不留诺艾尔行
    modes_n = len(modes)
...
def make_icon():
    ...
    # 渐变底: 上深玫红 -> 下深玫褐(与皮肤 accent #b03a35 同源)
    top = (176, 58, 53)     # #b03a35 诺艾尔缎带的深玫红
    bottom = (46, 33, 28)   # #2e211c 深玫褐
    for y in range(S):
        t = y / (S - 1)
