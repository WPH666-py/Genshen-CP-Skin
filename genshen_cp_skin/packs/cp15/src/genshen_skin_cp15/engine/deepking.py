...
def pkg_root():
    """包目录 .../genshen_skin_cp15(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp15.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #eef1f8;
  --bg-layer-3: #dde3f0;
  --label-primary: #1f1b2e;
  --label-secondary: #4b4463;
  --label-tertiary: #7d7794;
  --brand-primary: #8fbde0;
  --accent: #8fbde0;
  --primary: #8fbde0;
  --interactive-bg-hover-solid: #e0eaf5;
  --interactive-bg-active: #c7dcee;
  --border-l1: #dde6f1;
  --border-l2: #c2d3e5;
  --border: #c2d3e5;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP15</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP15", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP15", mascot.get("dark")),
    }
