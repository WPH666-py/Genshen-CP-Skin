...
本目录的 `genshen-cp5.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp5.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp5 deepking          # 导出 ~/.genshen-cp5/deepking/*.skin.json
  ```
...
```bash
genshen-cp5 deepking --check    # 应输出「通过」
genshen-cp5 deepking --what     # 看转换器实际会取到什么值
```
...
- `mascot-cp5-light.jpg` —— 取「甜点」那张的上半身特写, 配浅色界面
- `mascot-cp5-dark.jpg` —— 取「纸牌」那张的夜街双人, 配深色界面
