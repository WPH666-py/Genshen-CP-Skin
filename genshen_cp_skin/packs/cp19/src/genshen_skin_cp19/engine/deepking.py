...
def pkg_root():
    """包目录 .../genshen_skin_cp19(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp19.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #f3f0e9;
  --bg-layer-3: #e4dfd4;
  --label-primary: #262232;
  --label-secondary: #4f4a5e;
  --label-tertiary: #837d90;
  --brand-primary: #c9a44a;
  --accent: #c9a44a;
  --primary: #c9a44a;
  --interactive-bg-hover-solid: #f4ead6;
  --interactive-bg-active: #e9dab8;
  --border-l1: #ece5d6;
  --border-l2: #d8cdb5;
  --border: #d8cdb5;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP19</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP19", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP19", mascot.get("dark")),
    }
