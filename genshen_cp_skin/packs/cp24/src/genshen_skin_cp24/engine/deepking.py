...
def pkg_root():
    """包目录 .../genshen_skin_cp24(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp24.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #f5eef1;
  --bg-layer-3: #e6d9e0;
  --label-primary: #2b1a21;
  --label-secondary: #553b45;
  --label-tertiary: #8a6c78;
  --brand-primary: #b62230;
  --accent: #b62230;
  --primary: #b62230;
  --interactive-bg-hover-solid: #f7dfe3;
  --interactive-bg-active: #eec3ca;
  --border-l1: #ecdee4;
  --border-l2: #d6c1cb;
  --border: #d6c1cb;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP24</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP24", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP24", mascot.get("dark")),
    }
