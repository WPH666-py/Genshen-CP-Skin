...
try:
    from genshen_skin_cp24.engine import skin_core as sc
...
def make_thumbs():
    ...
    # 拼版总览图: 按实际样式数量排成一行(本套件 3 个), 不留绫华行
    modes_n = len(modes)
...
def make_icon():
    ...
    # 渐变底: 上深绯 -> 下深绯墨(与皮肤 accent #b62230 同源)
    top = (182, 34, 48)     # #b62230 红伞的深绯
    bottom = (43, 26, 33)   # #2b1a21 深绯墨
    for y in range(S):
        t = y / (S - 1)
