...
try:
    from genshen_skin_cp21.engine import skin_core as sc
...
def make_thumbs():
    ...
    # 拼版总览图: 按实际样式数量排成一行(本套件 3 个), 不留魈行
    modes_n = len(modes)
...
def make_icon():
    ...
    # 渐变底: 上青绿 -> 下深墨绿(与皮肤 accent #3f7d78 同源)
    top = (63, 125, 120)     # #3f7d78 魈的青绿
    bottom = (33, 42, 36)   # #212a24 深墨绿
    for y in range(S):
        t = y / (S - 1)
