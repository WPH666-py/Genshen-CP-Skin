...
本目录的 `genshen-cp9.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp9.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp9 deepking          # 导出 ~/.genshen-cp9/deepking/*.skin.json
  ```
...
```bash
genshen-cp9 deepking --check    # 应输出「通过」
genshen-cp9 deepking --what     # 看转换器实际会取到什么值
```
...
- `mascot-cp9-light.jpg` —— 取「湖畔」那张的樱吹雪, 配浅色界面
- `mascot-cp9-dark.jpg` —— 取「石阶」那张的社殿构图, 配深色界面
