...
def pkg_root():
    """包目录 .../genshen_skin_cp9(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp9.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #f3eef8;
  --bg-layer-3: #e5dcef;
  --label-primary: #2a2138;
  --label-secondary: #574a68;
  --label-tertiary: #877996;
  --brand-primary: #e8734a;
  --accent: #e8734a;
  --primary: #e8734a;
  --interactive-bg-hover-solid: #f7e6ea;
  --interactive-bg-active: #eed2da;
  --border-l1: #ebdee8;
  --border-l2: #dbc8da;
  --border: #dbc8da;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP9</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP9", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP9", mascot.get("dark")),
    }
