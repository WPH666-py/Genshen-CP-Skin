...
本套件有两张素材(神里绫华、宵宫), 每张三种摆法, 共 6 种壁纸:

  genshen-cp9                 # 默认: single1 卡片式(模糊背景 + 居中圆角卡片)
  genshen-cp9 cover           # 满屏: cover 裁切铺满整屏, 无边框
  genshen-cp9 showall         # 完整: 等比放进纯色底, 一个像素都不裁
  genshen-cp9 1               # 等价于 single1(兼容按张编号的写法)
  genshen-cp9 list            # 列出所有可切换样式
  genshen-cp9 random          # 随机来一张
  genshen-cp9 all             # 生成全部样式到 ~/.genshen-cp9/wallpapers
  genshen-cp9 all --out DIR   # 生成到指定目录(JetBrains 背景图用)
  genshen-cp9 cycle 30        # 每 30 分钟自动随机换壁纸
  genshen-cp9 switcher        # 打开可视化切换器
  genshen-cp9 pet             # 启动桌面桌宠
  genshen-cp9 deepking        # 生成 DeepKing 界面皮肤 + 离线预览
  genshen-cp9 copy            # 只合成不设置
...
def _cmd_list(args):
    ...
    if sc.count() > 1:
        ...
        print("  用法: %s [card|cover|showall]  |  %s switcher  |  %s pet"
              % (C.APP_SLUG, C.APP_SLUG, C.APP_SLUG))
        print("  说明: 两张素材 x 三种摆法 = 6 种壁纸, 用序号选图、用 single/cover/showall 选摆法。")
    return 0
...
def build_parser():
    ...
    # 每一种模式都注册成子命令。
    # argparse 的子命令是"贪婪"的: 一旦注册了子解析器, `genshen-cp9 cover`
    ...
    # 覆盖掉子解析器 set_defaults 设的 mode, 于是 `genshen-cp9 cover2` 反而
