...
def pkg_root():
    """包目录 .../genshen_skin_cp11(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp11.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #f9ece1;
  --bg-layer-3: #f0d9c7;
  --label-primary: #2b1a16;
  --label-secondary: #5e4038;
  --label-tertiary: #8d6f64;
  --brand-primary: #b8352f;
  --accent: #b8352f;
  --primary: #b8352f;
  --interactive-bg-hover-solid: #f7e0d4;
  --interactive-bg-active: #eec8b4;
  --border-l1: #f0dccd;
  --border-l2: #dfc2ad;
  --border: #dfc2ad;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP11</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP11", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP11", mascot.get("dark")),
    }
