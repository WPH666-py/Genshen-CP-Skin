...
本目录的 `genshen-cp11.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp11.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp11 deepking          # 导出 ~/.genshen-cp11/deepking/*.skin.json
  ```
...
```bash
genshen-cp11 deepking --check    # 应输出「通过」
genshen-cp11 deepking --what     # 看转换器实际会取到什么值
```
...
- `mascot-cp11-light.jpg` —— 取「夜市」那张的灯笼双人, 配浅色界面
- `mascot-cp11-dark.jpg` —— 取「锅巴」那张的暖光火锅, 配深色界面
