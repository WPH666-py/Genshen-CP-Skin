...
def pkg_root():
    """包目录 .../genshen_skin_cp18(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp18.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #eeeef4;
  --bg-layer-3: #dcdcea;
  --label-primary: #1e1c33;
  --label-secondary: #474568;
  --label-tertiary: #78769a;
  --brand-primary: #3d3a6b;
  --accent: #3d3a6b;
  --primary: #3d3a6b;
  --interactive-bg-hover-solid: #e3e2f0;
  --interactive-bg-active: #cdcbe4;
  --border-l1: #e0dfec;
  --border-l2: #c7c5dd;
  --border: #c7c5dd;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP18</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP18", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP18", mascot.get("dark")),
    }
