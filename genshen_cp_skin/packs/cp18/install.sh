#!/usr/bin/env bash
# 原神 CP 壁纸套件 18 · 散兵 x 万叶 —— 安装器 (macOS / Linux)
set -e
cd "$(dirname "$0")"
...
if "$PY" -c "import genshen_skin_cp18" >/dev/null 2>&1; then
  echo "[1/2] 已检测到 genshen-skin-cp18"
  RUN=("$PY" -m genshen_skin_cp18.engine.autoinstall)
else
  echo "[1/2] 以仓库源码方式运行(无需预先 pip 安装)"
  RUN=(env PYTHONPATH="$(pwd)/src" "$PY" -m genshen_skin_cp18.engine.autoinstall)
fi
